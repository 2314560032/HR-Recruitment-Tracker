...
function AppContent() {
  ...
  const [tourForcedTab, setTourForcedTab] = useState<"candidates" | "applications" | undefined>(undefined);
  const [tourForcedDept, setTourForcedDept] = useState<string | undefined>(undefined);
  const [tourForcedJobId, setTourForcedJobId] = useState<string | undefined>(undefined);

  const handleStartTour = () => {
    setTourActive(true);
    setTourStep(0);
    setTourForcedTab(undefined);
    setTourForcedDept(undefined);
    setTourForcedJobId(undefined);
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] text-[#13241C] flex flex-col md:flex-row relative">
      ...
      {/* Mobile Floating Utility Stack (Onboarding Tour & Sync Database) */}
      {currentScreen === "dashboard" && !selectedApplicationId && (
        <div className="md:hidden fixed bottom-20 right-4 flex flex-col gap-3 z-45">
          {/* Start Guided Onboarding Tour (Circle Icon) */}
          <button
            id="btn-start-onboarding-mobile"
            onClick={handleStartTour}
            title="Start guided onboarding tour"
            className="w-12 h-12 rounded-full border border-emerald-400 bg-emerald-50 hover:bg-emerald-100 text-[#2D6A4F] shadow-lg flex items-center justify-center transition-all hover:scale-105 active:scale-95 cursor-pointer"
          >
            <Slicon name="sparkles" size={18} />
          </button>

          {/* Synchronize Database (Circle Icon) */}
          <button
            onClick={() => reloadState()}
            disabled={loading}
            title="Synchronize Database"
            className="w-12 h-12 rounded-full border border-gray-200 bg-white hover:bg-slate-50 text-gray-600 shadow-lg flex items-center justify-center transition-all hover:scale-105 active:scale-95 cursor-pointer"
          >
            <Slicon name="refresh" size={18} className={loading ? "animate-spin" : ""} />
          </button>
        </div>
      )}
      ...
      <main className="flex-grow flex flex-col min-h-screen pb-20 md:pb-6 overflow-x-hidden">
        ...
        <header className="h-18 px-5 md:px-8 border-b border-[#F0FAF4] bg-white flex items-center justify-between sticky top-0 z-30 shadow-sm shrink-0 select-none">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Launch Guided Tour (F1.3) */}
            {currentScreen === "dashboard" && !selectedApplicationId && (
              <button
                id="btn-start-onboarding"
                onClick={handleStartTour}
                className="hidden md:flex py-2 px-4 rounded-xl border border-emerald-400 bg-emerald-50 hover:bg-emerald-100 text-[#2D6A4F] text-xs font-bold uppercase tracking-wider shadow-sm transition-all items-center gap-1.5 cursor-pointer min-h-[40px]"
              >
                <Slicon name="sparkles" size={14} />
                Start guided onboarding tour
              </button>
            )}
          </div>
        </header>
                  ...
                  case "feedback":
                    return (
                      <FeedbackScreen
                        ...
                        forcedDept={tourForcedDept}
                        forcedJobId={tourForcedJobId}
                      />
                    );
                  case "ai-generator":
                    return (
      ...
      {/* Guided Onboarding overlay layer */}
      {tourActive && (
        <OnboardingTour
          isActive={tourActive}
          onClose={handleCloseTour}
          currentStep={tourStep}
          setStep={(step) => setTourStep(step)}
          setSelectedPipelineJobId={(id) => setSelectedPipelineJobId(id)}
          setTourForcedTab={(tab) => setTourForcedTab(tab)}
          setTourForcedDept={(dept) => setTourForcedDept(dept)}
          setTourForcedJobId={(jobId) => setTourForcedJobId(jobId)}
          setSelectedFeedbackAppId={(id) => setSelectedFeedbackAppId(id)}
